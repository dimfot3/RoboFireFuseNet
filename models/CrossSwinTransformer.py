...
class CrossWMSA(nn.Module):
    """ Self-attention module in Swin Transformer
    """

    def __init__(self, input_dim, output_dim, head_dim, window_size, type):
        super(CrossWMSA, self).__init__()
    ...
    def forward(self, q, k, v):
        ...
        """
        h_windows = np.sqrt(q.size(2)).astype('int')
...
class CrossBlock(nn.Module):
    def __init__(self, input_dim, output_dim, head_dim, window_size, drop_path, type='W', input_resolution=None):
        ...
        self.msa = CrossWMSA(input_dim, input_dim, head_dim, window_size, self.type)
...
class CrossSwinTransformer(nn.Module):
    ...
    def __init__(self, config=[2,2], dim=96, drop_path_rate=0.2, input_resolution=252):
        ...
        for i_tf, n_tf in enumerate(self.config):
            for i in range(n_tf):
                ...
                self.tf_arr.append(CrossBlock((2**i_tf) * dim, (2**i_tf) * dim, self.head_dim, self.window_size, 
    ...
    def forward(self, x, q_arr, k_arr, v_arr):
        for i_tf, n_tf in enumerate(self.config):
            x = self.prestages[i_tf](x)
            for i in range(n_tf):
                tf_idx = sum(self.config[:i_tf]) + i
                x = self.tf_arr[tf_idx](x, q_arr[tf_idx], k_arr[tf_idx], v_arr[tf_idx])
        return x
