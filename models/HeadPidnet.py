...
class PIDNetHead(nn.Module):
    ...
    def __init__(self, m=2, n=3, num_classes=19, planes=64, ppm_planes=96, head_planes=128, augment=True, channels=3):
        super(PIDNetHead, self).__init__()
...
if __name__ == '__main__':
    ...
    model = model = PIDNetHead(m=2, n=3, num_classes=2, planes=32, ppm_planes=96, head_planes=128, augment=False, channels=3)
    model.eval()
    model.to(device)    
    input = torch.randn(4, 64, 32, 32).to(device)
    summary(model, input)
